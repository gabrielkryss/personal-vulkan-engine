...
uint32_t
vkUtil::findMemoryTypeIndex(vk::PhysicalDevice physicalDevice,
  ...
  */
  ...
  for (uint32_t i = 0; i < memoryProperties.memoryTypeCount; i++) {
    ...
    // bit i of supportedMemoryIndices is set if that memory type is supported
    // by the device
    bool supported{static_cast<bool>(supportedMemoryIndices & (1 << i))};

    // propertyFlags holds all the memory properties supported by this memory
    // type
