...
void Engine::make_swapchain() {
  ...
  vkInit::SwapChainBundle bundle =
      vkInit::create_swapchain(device, physicalDevice, surface, width, height);
...
/**
 * The swapchain must be recreated upon resize or minimization, among other
 * cases
 */
...
void Engine::make_assets() {
  ...
  meshDescriptorPool = vkInit::make_descriptor_pool(
      device, static_cast<uint32_t>(filenames.size()), bindings);
...
void Engine::record_draw_commands(vk::CommandBuffer commandBuffer,
  ...
  // Triangles
  render_objects(commandBuffer, meshTypes::TRIANGLE, startInstance,
                 static_cast<uint32_t>(scene->trianglePositions.size()));

  // Squares
  render_objects(commandBuffer, meshTypes::SQUARE, startInstance,
                 static_cast<uint32_t>(scene->squarePositions.size()));

  // Stars
  render_objects(commandBuffer, meshTypes::STAR, startInstance,
                 static_cast<uint32_t>(scene->starPositions.size()));
...
void Engine::render_objects(vk::CommandBuffer commandBuffer,
                            meshTypes objectType, uint32_t &startInstance,
...
Engine::~Engine() {
  ...
  * void Instance::destroy( Optional<const
  VULKAN_HPP_NAMESPACE::AllocationCallbacks> allocator = nullptr, Dispatch const
  & d = ::vk::getDispatchLoaderStatic())
  */
  instance.destroy();
