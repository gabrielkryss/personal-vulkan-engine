...
/**
        ...
        \param specification the struct holding input data, as specified at the
   top of the file.
        \returns the bundle of data structures created
*/
GraphicsPipelineOutBundle
create_graphics_pipeline(GraphicsPipelineInBundle &specification);
...
make_shader_info(const vk::ShaderModule &shaderModule,
...
make_viewport_state(const vk::Viewport &viewport, const vk::Rect2D &scissor);
...
*/
vk::PipelineLayout
make_pipeline_layout(vk::Device device,
                     vk::DescriptorSetLayout descriptorSetLayout);
...
/**
        ...
        \param device the logical device
        \param swapchainImageFormat the image format chosen for the swapchain
   images
...
make_color_attachment(const vk::Format &swapchainImageFormat);
...
/**
        ...
        \param colorAttachmentRef a reference to a color attachment for the
   color buffer
        \returns a description of the subpass
*/
vk::SubpassDescription
make_subpass(const vk::AttachmentReference &colorAttachmentRef);
...
make_renderpass_info(const vk::AttachmentDescription &colorAttachment,
...
create_graphics_pipeline(GraphicsPipelineInBundle &specification) {
  ...
  vk::ShaderModule vertexShader =
      vkUtil::createModule(specification.vertexFilepath, specification.device);
  ...
  vk::RenderPass renderpass =
      make_renderpass(specification.device, specification.swapchainImageFormat);
